...
def video_detection(path_x, model_type='ppe'):
    """Process video or image with object detection"""
    video_capture = path_x
    print(f"Video detection called with path: {path_x}, model type: {model_type}")
    
    # If path_x is empty or None, create an error frame
    if not path_x:
        error_img = create_error_frame("No file specified. Please upload a video or image.")
        yield error_img
        return
    ...
    try:
        # Create a Webcam Object for video file
        print(f"Opening video capture for: {video_capture}")
        cap = cv2.VideoCapture(video_capture)
        
        # Check if video opened successfully
        if not cap.isOpened():
            print(f"Error: Could not open video source {video_capture}")
            error_img = create_error_frame(f"Could not open video file. Check if the file exists and is a valid video format.")
            yield error_img
            return
            ...
            while True:
                ...
                if not success:
                    ...
                    if frame_count > 10:
                        ...
                        error_img = create_error_frame("Video playback ended or error occurred.")
                        yield error_img
                        break
                ...
                try:
                    ...
                    # Don't stop the video, just show this frame without detections
                    cv2.putText(img, f"Detection error: {str(e)[:30]}", (10, 30), 
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
                
                # Add watermark with detection info
                cv2.putText(img, f"Model: {model_type.upper()}", (10, 30), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
        ...
        print(f"General error in video detection: {str(e)}")
        error_img = create_error_frame(f"General error: {str(e)}")
        yield error_img
